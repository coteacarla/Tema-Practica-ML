...
Pentru rezolvarea problemei de clasificare binară (predicția sosului \textit{Crazy Sauce}), a fost implementată Regresia Logistică folosind metoda Gradient Descent.
...
Un aspect crucial pentru eficiența algoritmului de optimizare a fost preprocesarea datelor prin standardizare. Am transformat variabilele de intrare astfel încât să aibă media 0 și deviația standard 1. Această uniformizare a suprafeței de eroare a permis utilizarea unei rate de învățare agresive ($learning\_rate = 0.5$) fără riscul divergenței. 
...
Ulterior, am setat o rată de învățare agresivă ($learning\_rate = 0.25$) combinată cu mecanismul de Early Stopping (patience = 5 epoci), însă, în mod surprinzător, acesta nu a fost declanșat pe parcursul experimentelor. 
...
\subsection{Evaluarea Modelului și Interpretarea Rezultatelor}
...
\begin{table}[h]
    ...
    \begin{tabular}{lccc}
        \toprule
        \textbf{Metrică} & \textbf{Custom Implementation} & \textbf{sklearn Baseline} & \textbf{Diferență} \\
        \midrule
...
Fiecare dintre cele 8 modele a convers dupa 15000 de iterații (max(Loss) $\approx 0.0024$).
